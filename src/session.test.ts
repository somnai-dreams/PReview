...
test('message boundary rejects cross-account state and stale page sessions before any write', async () => {
  let account = 'alice', writes = 0
  const parent = { postMessage(message: unknown) { replies.push(message as { result?: { session: { account: string }; values: unknown[] }; error?: string }) } }
  const replies: { result?: { session: { account: string }; values: unknown[] }; error?: string }[] = []
  ...
  runInNewContext(source + '\n cells.set("draft",[{id:"draft",kind:"state",schema:{root:0,nodes:[{kind:"primitive",name:"string"}]},read:()=>"private draft",write:onWrite}]); markMounted()', {
    ...
    parent, window: {}, sessionCheck: () => Promise.resolve({ account, environment: 'test' }), onWrite: () => writes++,
