...
type ReviewerOptions = { port: number; builds: string[]; tls?: { key: string; cert: string } }

export function startReviewer(options: ReviewerOptions) {
const port = localPort(String(options.port))
const builds = options.builds.map(localURL)
...
button{font:inherit;padding:8px 14px;border:1px solid #777;border-radius:7px;background:#292b30;color:inherit;cursor:pointer}
...
</style></head><body><header><strong>PReview</strong><span id="status">Canvas edits stay in each build</span></header>
<script type="module">
...
for (let index = 0; index < origins.length; index++) {
  ...
  button.textContent = origins[index];
...
addEventListener('message', event => {
  ...
  for (let target = 0; target < frames.length; target++) {
    ...
    preparations[target] = call(target, 'prepare', event.data.history).then(() => { preparationErrors[target] = null; }, error => { preparationErrors[target] = error; });
  }
});
addEventListener('message', event => {
...
function call(index, operation, snapshot) {
  return new Promise((resolve, reject) => {
    const id = ++sequence;
    const timer = setTimeout(() => { pending.delete(id); reject(Error('Build did not respond')); }, 5000);
    pending.set(id, { index, resolve, timer });
