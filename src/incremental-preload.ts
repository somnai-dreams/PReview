...
addEventListener('DOMContentLoaded', () => {
  ...
  const update = () => { output.textContent = JSON.stringify({ ...cache.stats(), generated: generated.stats(), initializationMs, heapBytes: (performance as Performance & { memory?: { usedJSHeapSize: number } }).memory?.usedJSHeapSize ?? null }, null, 2) }
