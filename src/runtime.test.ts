...
test('runtime applies native routing before repair and restores scroll by a visible anchor',async()=>{
 ...
 expect((await b.request('prepare',{...a.runtime.history(),session:null}))?.result).toHaveProperty('navigated',true)
 expect(route.read()).toEqual({page:'detail'})
 const result=await transfer(a,b)
 expect(result.report.rejected).toEqual([]);expect(b.histories.at(-1)).toEqual({state:{page:'detail'},path:'/jobs/one'});expect(JSON.stringify(restored)).toBe('[0,60]');expect(result.report.scrollRestored).toEqual([{id:'feed',method:'anchor'}])
})
