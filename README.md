...
Numeric identities use small array blocks instead of a hash entry per object. Incoming native objects share the live graph's object table while being validated; temporary aliases are removed when that receive finishes. Empty identity blocks are released with their last object. Memory still grows with the supported graph in each build, so very large first transfers remain expensive.

With complete write observation, warm transfers send changed container bodies and references to already shared objects. Bulk changes use the ordinary native graph instead of constructing a second JavaScript graph for delta encoding: when the upper bound on changed or unshared containers exceeds 4,096 and half the owned graph, the session chooses a full packet. The counts are maintained with the existing object ownership and acknowledgement lifecycle, so choosing a transport does not scan unchanged subtrees. Validation proofs belong to live object revisions under the generated schema. A tracked mutation invalidates that object and its ancestors; unchanged descendants retain their proofs. Simple property assignments through a proven local receiver compare the incoming value with its existing own data property before invalidating it. The original assignment still executes, so a repeated equal value needs no dirty mark, repair copy or validation work. Computed receivers, destructuring, compound assignments and opaque scopes retain conservative observation. Unsupported syntax or detected unobserved execution forces fresh full validation. Full checks retain one temporary native snapshot during React repair so unobserved effect writes are still detected. It is discarded after the commit; it is not persistent history.
...
- Routing transfers through the same observed state cells as the rest of the app. Its History API journal accompanies that state and is applied after the first React commit. Source navigation does not pre-navigate hidden builds or trigger their route-specific data loads. Unobserved router state and arbitrary browser navigation behavior are outside this mechanism; local back/forward still dispatches `popstate`.
