...
With incremental observation, capture constructs and validates the owned snapshot together and records its correspondence with live objects. Restoration records the objects it writes in that same index before React effects run, allowing checks and repairs to reuse unchanged subgraphs. The [large-transfer measurements](docs/large-transfers.md) document the improvement and remaining memory cost.
